...
export function useRenameEngine() {
  ...
  function generateNewName(
    ...
    switch (mode) {
      ...
      case 'number':
        return handleNumber(nameWithoutExt, ext, params as NumberParams, index || 0)
      case 'delete':
  ...
  function handleReplace(originalName: string, params: ReplaceParams): string {
    ...
    return originalName.replace(new RegExp(escapeRegExp(params.fromStr), 'g'), params.toStr)
  }

  // 添加前缀/后缀处理
  function handleAdd(nameWithoutExt: string, ext: string, params: AddParams): string {
  ...
  // 删除字符处理
  function handleDelete(nameWithoutExt: string, ext: string, params: DeleteParams): string {
    if (params.count <= 0 || params.startPos <= 0) return `${nameWithoutExt}${ext}`
    ...
    if (params.fromLeft) {
      ...
      // 从右侧删除
      const rightStartIndex = Math.max(0, result.length - startIndex - params.count + 1)
  ...
  // 执行重命名
  async function executeRename(): Promise<{ success: boolean; errors: string[] }> {
  ...
  // 撤回最后一次操作
  async function undoLastOperation(): Promise<{ success: boolean; errors: string[] }> {
