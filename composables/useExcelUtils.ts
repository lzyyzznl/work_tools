...
export function useExcelUtils() {
  ...
  function exportRulesToExcel(rules: Rule[], filename?: string) {
    try {
      ...
      // 生成文件名
      const defaultFilename = `rules-export-${new Date().toISOString().split('T')[0]}.xlsx`;
   ...
   * 从Excel文件导入规则
   */
  function importRulesFromExcel(file: File): Promise<{ success: boolean; rules?: Rule[]; error?: string }> {
          ...
          jsonData.forEach((row, index) => {
            try {
              ...
              const rule: Rule = {
                ...
                code: row.code.trim(),
                thirtyD: (row['30d'] || 'N').toString().toUpperCase(),
                matchRules: row.match_rules.split(';').map(r => r.trim()).filter(r => r),
                source: 'user'
              };
              
              // 验证30d字段
              if (!['Y', 'N'].includes(rule.thirtyD)) {
                rule.thirtyD = 'N';
              }
   ...
   * 导出匹配结果到Excel文件
   */
  function exportMatchResultsToExcel(results: ExcelMatchResult[], filename?: string) {
    try {
      // 创建工作簿
      const wb = XLSX.utils.book_new();
      ...
      // 生成文件名
      const defaultFilename = `file-match-results-${new Date().toISOString().split('T')[0]}.xlsx`;
      const finalFilename = filename || defaultFilename;
  ...
  function createRuleTemplate() {
    ...
    return exportRulesToExcel(templateData.map(item => ({
