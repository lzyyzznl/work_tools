...
class CustomKeySequenceEdit(QKeySequenceEdit):
    ...
    def paintEvent(self, event):
        ...
        if self.keySequence().isEmpty() and not self.is_focused:
            ...
            painter.drawText(
                rect, Qt.AlignLeft | Qt.AlignVCenter, self.placeholder_text
            )
        ...
        if reply == QMessageBox.Yes:
            for action_name, default_shortcut in self.default_shortcuts.items():
                if action_name in self.shortcut_editors:
                    self.shortcut_editors[action_name].setKeySequence(
        ...
        for action_name, shortcut in shortcuts.items():
            if shortcut and shortcut != "":
                if shortcut in used_shortcuts:
                    conflicts.append(
                        f"'{shortcut}' 在 '{action_name}' 和 '{used_shortcuts[shortcut]}' 中重复"
                    )
                else:
...
class FileRenamer(QMainWindow):
    ...
    def setup_shortcuts(self):
        ...
        shortcut_mapping = {
            ...
            "复制选中数据": (self.file_table, self.copy_selected_cells),
        }
    ...
    def setup_apple_fonts(self):
        ...
        self.setStyleSheet(
            ...
            QTableWidget, QTableView, QHeaderView {{
                ...
                font-weight: 400;
                color: #1d1d1f;
                selection-background-color: rgba(0, 95, 204, 0.4);
            ...
            QTableWidget::item:selected {{
                ...
                color: #1d1d1f;
                border: 1px solid rgba(0, 95, 204, 0.6);
            }}
            
            QTableWidget::item:selected:hover {{
                background-color: rgba(0, 95, 204, 0.5);
                border: 1px solid rgba(0, 95, 204, 0.8);
            }}
    ...
    def get_config_file_path(self):
        ...
        if system == "Windows":
            ...
            config_dir = os.path.join(
                home_dir, "Library", "Application Support", "FileRenamerTool"
            )
    ...
    def create_toolbar(self):
        ...
        def get_icon(icon_name):
            ...
            if os.path.exists(icon_path):
                ...
                return style.standardIcon(
                    icon_mapping.get(icon_name, QStyle.SP_ComputerIcon)
                )
        ...
        add_action(
            ...
            "添加文件",
            self.add_files,
            self.shortcuts.get("添加文件", "Ctrl+O"),
        )
        add_action(
            get_icon("添加文件夹"),
            "添加文件夹",
            self.add_folder,
            self.shortcuts.get("添加文件夹", "Ctrl+Shift+O"),
        )
        ...
        add_action(
            ...
            self.shortcuts.get("预览更改", "F6"),
        )
        add_action(
            get_icon("重置参数"),
            "重置参数",
            self.reset_parameters,
            self.shortcuts.get("重置参数", "Ctrl+R"),
        )
        toolbar.addSeparator()
        ...
        add_action(
            ...
            self.shortcuts.get("清空列表", "Ctrl+Delete"),
        )
        add_action(
        ...
        add_action(
            ...
            "使用说明",
            self.show_help,
            self.shortcuts.get("显示帮助", "F1"),
        )

        # 添加设置菜单项
        toolbar.addSeparator()
        add_action(
            get_icon("设置快捷键"), "快捷键设置", self.show_shortcut_settings, ""
        )
    ...
    def create_operation_tabs(self):
        ...
        replace_layout.addWidget(QLabel("将:"))
        ...
        prefix_radio = QRadioButton("添加为前缀")
        ...
        add_layout.addWidget(prefix_radio)
        add_layout.addWidget(suffix_radio)
        ...
        num_prefix_radio = QRadioButton("前缀")
        num_prefix_radio.setChecked(True)
        num_suffix_radio = QRadioButton("后缀")
        self.number_position_group.addButton(num_prefix_radio, 0)
        self.number_position_group.addButton(num_suffix_radio, 1)
        self.number_position_group.buttonClicked.connect(self.start_preview_timer)
        row1_layout.addWidget(QLabel("序号位置:"))
        row1_layout.addWidget(num_prefix_radio)
        row1_layout.addWidget(num_suffix_radio)
        ...
        row1_layout.addWidget(QLabel("连接符:"))
        row1_layout.addWidget(self.separator_input)
        row1_layout.addStretch()

        row2_layout = QHBoxLayout()
        self.start_number = QLineEdit("1")
        self.start_number.setFixedWidth(80)
        self.number_digits = QLineEdit("2")
        self.number_digits.setFixedWidth(80)
        self.number_step = QLineEdit("1")
        self.number_step.setFixedWidth(80)
        # Add auto-preview listeners
        ...
        row2_layout.addWidget(QLabel("起始数字:"))
        row2_layout.addWidget(self.start_number)
        row2_layout.addSpacing(20)
        row2_layout.addWidget(QLabel("数字位数(不足补0):"))
        row2_layout.addWidget(self.number_digits)
        row2_layout.addSpacing(20)
        row2_layout.addWidget(QLabel("步长:"))
        row2_layout.addWidget(self.number_step)
        row2_layout.addStretch()

        number_layout.addLayout(row1_layout)
        number_layout.addLayout(row2_layout)
        ...
        left_radio = QRadioButton("从左开始")
        left_radio.setChecked(True)
        right_radio = QRadioButton("从右开始")
        self.delete_direction_group.addButton(left_radio, 0)
        self.delete_direction_group.addButton(right_radio, 1)
        self.delete_direction_group.buttonClicked.connect(self.start_preview_timer)
        row1_layout.addWidget(QLabel("删除方向:"))
        row1_layout.addWidget(left_radio)
        row1_layout.addWidget(right_radio)
        row1_layout.addStretch()

        row2_layout = QHBoxLayout()
        self.delete_start_pos = QLineEdit("1")
        self.delete_start_pos.setFixedWidth(80)
        self.delete_count = QLineEdit("1")
        self.delete_count.setFixedWidth(80)
        ...
        row2_layout.addWidget(QLabel("开始位置:"))
        row2_layout.addWidget(self.delete_start_pos)
        row2_layout.addSpacing(20)
        row2_layout.addWidget(QLabel("删除字符数:"))
        row2_layout.addWidget(self.delete_count)
        row2_layout.addStretch()

        delete_layout.addLayout(row1_layout)
        delete_layout.addLayout(row2_layout)
    ...
    def create_empty_state_widget(self):
        ...
        # Tips
        tips_label = QLabel(
            "💡 支持的快捷键：Ctrl+O (添加文件)、Ctrl+Shift+O (添加文件夹)"
        )
    ...
    def create_file_table(self):
        ...
        self.header_checkbox = QCheckBox("全选/全不选")
        self.header_checkbox.setChecked(False)
        self.header_checkbox.stateChanged.connect(
            lambda s: self.toggle_all_checkboxes(Qt.CheckState(s))
        )
        ...
        scaled_pixmap = help_pixmap.scaled(
        ...
        refresh_button.setToolTip(
            f"刷新文件列表，更新文件信息并移除已删除的文件 ({refresh_shortcut})"
        )
        ...
        # 连接点击空白处的事件，用于取消选择
        table.mousePressEvent = self.table_mouse_press_event
    ...
    def show_table_context_menu(self, position):
        ...
        # 通用操作（复制和导出）
        copy_action = menu.addAction("📋 复制选中数据")
        copy_shortcut = self.shortcuts.get("复制选中数据", "Ctrl+C")
        copy_action.setText(f"📋 复制选中数据 ({copy_shortcut})")
        copy_action.triggered.connect(self.copy_selected_cells)

        export_action = menu.addAction("📊 导出表格数据")
        export_action.triggered.connect(self.export_table_data)
        ...
        if clicked_item:
            ...
            menu.addSeparator()
            remove_action = menu.addAction("🗑️ 从列表中移除")
            ...
            refresh_action = menu.addAction("🔄 刷新文件列表")
            menu.addSeparator()
            remove_action = menu.addAction("🗑️ 从列表中移除选中项")
    ...
    def add_folder(self):
        ...
        if folder_path := QFileDialog.getExistingDirectory(self, "选择文件夹"):
            paths = [
                str(item) for item in Path(folder_path).iterdir() if item.is_file()
            ]
            for path in paths:
                self.add_file_to_table(path)
    ...
    def remove_selected_files(self):
        ...
        selected_rows = sorted(
            {idx.row() for idx in self.file_table.selectedIndexes()}, reverse=True
        )
    ...
    def preview_changes(self):
        ...
        for i, row in enumerate(rows_to_process):
            ...
            if "." in original_name:
                original_stem = original_name.rsplit(".", 1)[
                    0
            ...
            if new_stem and new_stem != original_stem:
                ...
                file_info["preview_name"] = (
                    ""  # No change, so no preview_name for execution
                )
        ...
        changed_count = sum(
            1 for row in rows_to_process if self.files_data[row]["preview_name"]
        )
        self.update_status(
            f"已为 {len(rows_to_process)} 个文件生成预览，其中 {changed_count} 个将被重命名。"
        )
    ...
    def execute_rename(self):
        ...
        rows_to_rename = [
            row
            for row in self.get_rows_to_process()
            if self.files_data[row]["preview_name"]
        ]
        if not rows_to_rename:
            QMessageBox.information(
                self, "无操作", "没有可供重命名的文件。请先生成预览。"
            )
            return
        ...
        if (
            QMessageBox.question(
                self,
                "确认操作",
                f"即将重命名 {len(rows_to_rename)} 个文件。是否继续？",
                QMessageBox.Yes | QMessageBox.No,
                QMessageBox.No,
            )
            == QMessageBox.No
        ...
        for row in rows_to_rename:
            ...
            try:
                if new_path.exists():
                    raise OSError("目标文件名已存在")
                os.rename(old_path, new_path)

                current_batch_history.append((str(old_path), str(new_path)))
                file_data.update(
                    path_obj=new_path, original_name=new_path.name, preview_name=""
                )
    ...
    def undo_last_operation(self):
        ...
        if (
            QMessageBox.question(
                self,
                "确认撤回",
                f"即将撤回 {len(last_batch)} 个文件的命名。是否继续？",
                QMessageBox.Yes | QMessageBox.No,
                QMessageBox.No,
            )
            == QMessageBox.No
            ...
            try:
                ...
                for file_data in self.files_data:
                    if str(file_data["path_obj"]) == new_path_str:
                        file_data.update(
                            path_obj=Path(old_path_str),
                            original_name=Path(old_path_str).name,
                            preview_name="",
                        )
                        break
                success += 1
    ...
    def _get_operation_params(self):
        ...
        if current_tab_index == 0:
            params.update(
                type="replace",
                from_str=self.replace_from.text(),
                to_str=self.replace_to.text(),
            )
    ...
    def _calculate_new_stem(self, original_stem, params, index):
        ...
        if op_type == "replace":
            ...
            num_str = (
                f"{params['start'] + (index * params['step']):0{params['digits']}d}"
            )
    ...
    def add_file_to_table(self, file_path):
        ...
        if path_obj.resolve() in (
            item["path_obj"].resolve() for item in self.files_data
        ):
            return
        ...
        self.files_data.append(
            {"path_obj": path_obj, "preview_name": "", "original_name": path_obj.name}
        )
        ...
        last_modified = QDateTime.fromSecsSinceEpoch(int(file_stat.st_mtime)).toString(
            "yyyy-MM-dd hh:mm:ss"
        )
    ...
    def get_rows_to_process(self):
        ...
        checked_rows = [
            ...
            if self.file_table.item(i, 0).checkState() == Qt.Checked
        ]
    ...
    def refresh_table_display(self):
        ...
        for item in temp_data:
            self.add_file_to_table(str(item["path_obj"]))
    ...
    def show_help(self):
        ...
        if (
            not help_file_path.exists() or True
        ):  # Always recreate to show current shortcuts
            # 生成快捷键帮助文本
...
在文件列表中右键点击可以：
- 复制选中数据：复制选中单元格到剪贴板，支持Ctrl+C快捷键
- 导出表格数据：导出所有数据到Excel或CSV文件
- 打开文件：直接打开选中的文件
...
- 支持多行多列选择：按住Ctrl或Shift键选择多个单元格
- 复制功能：选中单元格后右键复制或使用Ctrl+C，数据格式为CSV
- 导出功能：支持导出到Excel(.xlsx/.xls)或CSV格式
- 点击列标题可以排序
- 双击文件名或预览列可以直接打开文件
- 点击蓝色路径可以打开文件夹
- 执行结果显示操作成功/失败状态
...
""".format(
    ...
    def rename_single_file(self, row):
        ...
        new_name, ok = QInputDialog.getText(
            self, "重命名文件", f"新文件名:", text=old_name
        )
    ...
    def refresh_file_list(self):
        ...
        for row in range(
            ...
            if not file_path.exists():
                ...
                try:
                    ...
                    modification_time = QDateTime.fromSecsSinceEpoch(
                        int(stat_result.st_mtime)
                    )
                    size_bytes = stat_result.st_size

                    # Update table display
                    self.file_table.item(row, 4).setText(
                        modification_time.toString("yyyy-MM-dd hh:mm:ss")
                    )
        ...
        if missing_files:
            self.update_status(
                f"刷新完成。更新了 {refreshed_count} 个文件，移除了 {len(missing_files)} 个不存在的文件。"
            )
        else:
    ...
    def table_mouse_press_event(self, event):
        ...
        if item is None:
            ...
            self.file_table.setCurrentItem(None)
        else:
            # 点击的是有效项目，调用原始的鼠标按下事件
            QTableWidget.mousePressEvent(self.file_table, event)
    ...
    def copy_selected_cells(self):
        ...
        for item in selected_items:
            row = item.row()
            col = item.column()
            cells_by_position[(row, col)] = item.text()
        ...
        # 获取行和列的范围
        rows = sorted(set(pos[0] for pos in cells_by_position.keys()))
        cols = sorted(set(pos[1] for pos in cells_by_position.keys()))
        ...
        for row in rows:
            ...
            for col in cols:
                ...
                # 特殊处理：如果是复选框列(第0列)，转换为可读文本
                if col == 0:
                    checkbox_item = self.file_table.item(row, col)
                    if checkbox_item:
                        cell_text = (
                            "☑" if checkbox_item.checkState() == Qt.Checked else "☐"
                        )
                    else:
                        cell_text = ""
                row_data.append(cell_text)
            copy_lines.append(",".join(row_data))
        ...
        self.update_status(f"已复制 {cell_count} 个单元格到剪贴板。")
    ...
    def export_table_data(self):
        ...
        # 检查是否有选中的单元格
        selected_items = self.file_table.selectedItems()
        export_selected_only = len(selected_items) > 0

        try:
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            if export_selected_only:
                default_name = f"file_renamer_selected_data_{timestamp}.xlsx"
            else:
                default_name = f"file_renamer_all_data_{timestamp}.xlsx"
            ...
            if export_selected_only:
                ...
                # 创建选中列的标题
                selected_headers = [headers[col] for col in cols]
            ...
            self.update_status(
                f"已导出{data_type}数据({export_count}行)到: {file_path}"
            )
            ...
            reply = QMessageBox.question(
                self,
                "导出成功",
                f"已成功导出{data_type}数据({export_count}行)到:\n{file_path}\n\n是否打开文件？",
                QMessageBox.Yes | QMessageBox.No,
            )
