<template>
  ...
  >
    <!-- 标题栏 -->
    <div class="title-bar flex items-center justify-between px-6 py-4 bg-white border-b border-gray-200 shadow-sm">
      ...
      <div class="title-right flex items-center gap-4">
        <div class="stats flex items-center gap-6 text-sm text-gray-600">
          <div class="flex items-center gap-2">
            <span>文件:</span>
            <span class="font-semibold text-blue-600">{{ fileStore.fileStats.total }}</span>
          </div>
          <div class="flex items-center gap-2">
            <span>规则:</span>
    ...
    <div class="nav-tabs flex bg-white border-b border-gray-200">
      <button
        ...
        class="tab-button flex-1 px-6 py-4 text-center font-medium transition-colors relative"
        :class="activeTab === 'matcher'
      ...
      >
        <div class="flex items-center justify-center gap-2">
          <span>🎯</span>
          <span>文件匹配器</span>
          <kbd class="ml-2 px-2 py-1 text-xs bg-gray-200 text-gray-600 rounded">Ctrl+1</kbd>
        </div>
      </button>
      ...
      >
        <div class="flex items-center justify-center gap-2">
          <span>✏️</span>
          <span>文件重命名器</span>
          <kbd class="ml-2 px-2 py-1 text-xs bg-gray-200 text-gray-600 rounded">Ctrl+2</kbd>
        </div>
      </button>
    </div>
    ...
    >
      <div class="bg-white rounded-lg shadow-xl p-8 text-center">
        ...
        <div class="text-xl font-semibold text-gray-900 mb-2">释放文件到此处</div>
...
onMounted(() => {
  ...
  // 文件操作快捷键
  registerShortcut(commonShortcuts.selectFiles(async () => {
    try {
